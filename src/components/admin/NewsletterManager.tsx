  ...
  return (
    ...
    >
      ...
      <Dialog open={openCampaignDialog} onOpenChange={setOpenCampaignDialog}>
        <DialogContent className="bg-dark-500 border-dark-400 text-white">
          <DialogHeader>
            <DialogTitle>{isEditing ? 'Kampanya Düzenle' : 'Yeni Kampanya Oluştur'}</DialogTitle>
            <DialogDescription className="text-white/60">
              E-bülten kampanyası için gerekli bilgileri girin.
            </DialogDescription>
          </DialogHeader>
          
          <div className="space-y-4 pt-4">
            <div className="space-y-2">
              <label className="text-sm font-medium text-white">Kampanya Başlığı</label>
              <Input 
                value={currentCampaign.title || ''} 
                onChange={(e) => setCurrentCampaign({...currentCampaign, title: e.target.value})}
                className="bg-dark-400 border-dark-300 text-white"
                placeholder="Mayıs Ayı Bülteni"
              />
            </div>
            
            <div className="space-y-2">
              <label className="text-sm font-medium text-white">E-posta Konusu</label>
              <Input 
                value={currentCampaign.subject || ''} 
                onChange={(e) => setCurrentCampaign({...currentCampaign, subject: e.target.value})}
                className="bg-dark-400 border-dark-300 text-white"
                placeholder="ATY Dijital - Mayıs Ayı Gelişmeleri"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea 
                ...
                className="bg-dark-400 border-dark-300 text-white h-32 resize-none"
                placeholder="<h1>Başlık</h1><p>İçerik metni...</p>"
              />
              <p className="text-xs text-white/50">HTML formatında içerik girebilirsiniz.</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select 
                ...
                onValueChange={(value: 'draft' | 'scheduled' | 'sent') => setCurrentCampaign({...currentCampaign, status: value})}
              >
                <SelectTrigger className="bg-dark-400 border-dark-300 text-white">
                  <SelectValue placeholder="Durum seçin" />
                </SelectTrigger>
            ...
            {currentCampaign.status === 'scheduled' && (
              <div className="space-y-2">
                <label className="text-sm font-medium text-white">Gönderim Tarihi</label>
                <Input 
                  type="datetime-local"
                  value={currentCampaign.sendDate ? new Date(currentCampaign.sendDate).toISOString().slice(0, 16) : ''}
                  onChange={(e) => setCurrentCampaign({...currentCampaign, sendDate: e.target.value})}
                  className="bg-dark-400 border-dark-300 text-white"
                />
              </div>
            )}
          ...
          <DialogFooter className="flex justify-end gap-2 mt-4">
            <Button variant="outline" onClick={() => setOpenCampaignDialog(false)} className="border-dark-300 hover:bg-dark-400 text-white">
              İptal
            </Button>
            <Button onClick={handleSaveCampaign} className="bg-ignite hover:bg-ignite-700">
      ...
      <Dialog open={openTemplateDialog} onOpenChange={setOpenTemplateDialog}>
        <DialogContent className="bg-dark-500 border-dark-400 text-white">
          <DialogHeader>
            <DialogTitle>{isEditing ? 'Şablon Düzenle' : 'Yeni Şablon Oluştur'}</DialogTitle>
            <DialogDescription className="text-white/60">
              E-bülten için özelleştirilebilir bir şablon oluşturun.
            </DialogDescription>
          </DialogHeader>
          
          <div className="space-y-4 pt-4">
            <div className="space-y-2">
              <label className="text-sm font-medium text-white">Şablon Adı</label>
              <Input 
                value={currentTemplate.name || ''} 
                onChange={(e) => setCurrentTemplate({...currentTemplate, name: e.target.value})}
                className="bg-dark-400 border-dark-300 text-white"
                placeholder="Standart Bülten"
              />
            </div>
            
            <div className="space-y-2">
              <label className="text-sm font-medium text-white">Konu Şablonu</label>
              <Input 
                value={currentTemplate.subject || ''} 
                onChange={(e) => setCurrentTemplate({...currentTemplate, subject: e.target.value})}
                className="bg-dark-400 border-dark-300 text-white"
                placeholder="{{firma_adı}} - Aylık Bülten"
              />
            </div>
            
            <div className="space-y-2">
              <label className="text-sm font-medium text-white">HTML İçerik</label>
              <Textarea 
                value={currentTemplate.content || ''} 
                onChange={(e) => setCurrentTemplate({...currentTemplate, content: e.target.value})}
                className="bg-dark-400 border-dark-300 text-white h-32 resize-none"
                placeholder="<h1>{{bülten_başlığı}}</h1><p>{{içerik}}</p>"
              />
              <p className="text-xs text-white/50">Değişkenler için {{değişken_adı}} formatını kullanabilirsiniz.</p>
            </div>
          </div>
          
          <DialogFooter className="flex justify-end gap-2 mt-4">
            <Button variant="outline" onClick={() => setOpenTemplateDialog(false)} className="border-dark-300 hover:bg-dark-400 text-white">
              İptal
            </Button>
            <Button onClick={handleSaveTemplate} className="bg-ignite hover:bg-ignite-700">
              {isEditing ? 'Güncelle' : 'Oluştur'}
            </Button>
          </DialogFooter>
        </DialogContent>
